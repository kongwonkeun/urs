...
void setup()
{
    ...
    // bluetooth
    Serial1.begin(BLUETOOTH_AT_CMD_BAUDRATE);
    MY_WAIT();
    if (Serial1) {
        x_serial_bt = true;
    }
    MY_INFO("\n----2222----");
    MY_INFO("\ndisconnect vcc pin, press the button of module, and re-connect vcc");
    MY_INFO("\nwait some second until led goes to slow blinking");
    MY_INFO("\nrelease the button, and type your command");
    MY_INFO("\nAT");
    MY_INFO("\nAT+UART?");
    MY_INFO("\nAT+UART=115200,0,0");
    MY_INFO("\nAT+NAME=name");
